...
function DSSShell({ user, onLogout, dark, toggleDark }) {
  ...
  return (
    <div className={`min-h-screen flex ${d ? "bg-gray-950" : "bg-gray-50"}`}>

      {/* ── Sidebar ── */}
      <aside className={`w-64 flex flex-col flex-shrink-0 ${d ? "bg-gray-900 border-r border-gray-800" : "bg-white border-r border-gray-200"}`}>
      ...
      <main className="flex-1 overflow-auto flex flex-col">
        ...
        {/* Header */}
        <header className={`px-6 py-3 sticky top-0 z-10 flex-shrink-0 ${d ? "bg-gray-900 border-b border-gray-800" : "bg-white border-b border-gray-200"}`}>
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3 flex-shrink-0">
              <span className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${d ? "bg-blue-500/20 text-blue-400" : "bg-blue-100 text-blue-600"}`}>
                <Icon name={TABS.find(tb => tb.id === activeTab)?.icon} size={16} />
              </span>
              <h1 className={`text-sm font-semibold ${d ? "text-gray-100" : "text-gray-800"}`}>
            ...
            <div className="flex items-center gap-3 flex-1 justify-end">
              ...
              {/* Step pills — sadece küçük göster */}
        ...
        <div className="flex-1 overflow-hidden relative">
          {tabContent.map(({ id, el }) => (
            <div key={id}
                 className={`absolute inset-0 overflow-auto p-8 transition-opacity duration-150 ${
